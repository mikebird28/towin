...
PROCESS_FEATURE = ["hi_RaceID","ri_Year","hr_OrderOfFinish","li_WinOdds"]
...
def main():
    ...
    if mode == "normal":
        ...
        for i,(df1,df2) in enumerate(kf):
            if i == 0:
                model = light_gbm(df1,df2,categoricals,show_importance = True,score = score)
...
def light_gbm(df1,df2,categoricals,score = None,show_importance = True,typ = "classify"):
    if typ == "classify":
        x1,y1 = filter_df(df1)
        x2,y2 = filter_df(df2)
        params = {
...
def filter_df(df,target_variable = "is_win"):
    ...
    remove = ["hi_RaceID","ri_Year","hr_OrderOfFinish","hr_PaybackWin","hr_PaybackPlace","is_win","is_place","win_return","place_return","li_WinOdds"]
    features = []
    for c in df.columns:
        should_add = True
        for r in remove:
...
def evaluate(df,model,features,show_results = True):
    #df = pd.concat([df,y],axis = 1)
    x1,y1 = filter_df(df)
